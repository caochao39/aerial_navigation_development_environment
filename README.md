...
**Using CMU-Recon models (Ubuntu 20.04 only)**: Users can try reality models built by the CMU-Recon Team. In a terminal, go to the development environment folder, switch to the 'noetic-cmu-recon' branch, and then compile.
```
...
Now users can use the command line below to launch the system. Wait for the system to initialize in a few seconds, rendered RGB, depth, and semantic images will show in RVIZ.
